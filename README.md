...
- l'**armature minimale sous déformation gênée** (§7.3.2), qui gouverne les voiles et radiers massifs. Elle **ne rend aucun verdict** : elle donne une aire exigée, elle ne la compare à rien.
  ...
  **Correction du 2026-09-10.** `h_c,ef` employait les branches de la **flexion** sur un bridage **centré** : sur un radier de 1,30 m, 137,5 mm au lieu de 240, soit **43 % d'armature en moins**. En traction centrée la zone efficace **croît** avec l'épaisseur — après la première fissure traversante, les barres d'un élément épais sont trop espacées pour agir sur tout le cœur, et des fissures secondaires approfondissent leur zone d'influence bien au-delà de la zone de rive figée de la flexion.

  Les **deux approches sont calculées** — zone tendue entière et zone de béton tendu efficace — et **la plus petite est retenue**, la norme ne fixant pas de frontière nette entre élément mince et épais. L'approche épaisse est bornée par `k·f_ct,eff·A_ct/f_yk` : en deçà, l'acier plastifie à l'instant de la fissuration et la fissure devient unique et large au lieu de rester fine et répartie — exactement ce que cette armature existe pour empêcher. Elle n'applique **ni `k` ni `k_c`** : ils traduisent une réduction à l'échelle de la section entière, pas de la peau qui travaille, et les appliquer une seconde fois retirait 35 % d'acier.
  ...
  **Deux choix indépendants.** La **méthode** gouverne `h_c,ef` : texte de l'EN 1992-1-1 §7.3.2(3) (`2,5·d1`) ou branches selon `h/d1` de la pratique allemande. La **convention de `k`** est un paramètre d'annexe nationale : 1,00 → 0,65 (recommandé EC2) ou 0,80 → 0,50 (annexe allemande). Les lier interdirait « branches allemandes, `k` de l'annexe belge », légitime au Luxembourg.

  **Écart documenté au texte :** les branches selon `h/d1` ne figurent pas dans l'EN 1992-1-1, dont le §7.3.2(3) écrit `min(2,5(h−d) ; (h−x)/3 ; h/2)`. Elles viennent de la pratique allemande (Schneider ; Fingerloos, Hegger, Zilch). Le terme `(h−x)/3` est écarté dans les deux méthodes : il suppose un axe neutre de section fissurée **en flexion**, qui n'a pas de sens à l'instant de la première fissure sous bridage. En BE et au LU, la justification réglementaire reste l'EC2 et ses annexes NBN / ILNAS — le bloc le dit à l'écran.

  **Forcer une valeur.** `f_ct,eff`, `d1`, `h_c,ef`, `k`, `k_c`, `A_ct`, `A_c,ef` et `σ_s` peuvent être **imposés** à la main. Une valeur imposée court-circuite sa formule et alimente la suite de la chaîne. Elle est alors **marquée « IMPOSE »** à l'écran et dans la note : une note qui présenterait une valeur forcée comme calculée ne serait vérifiable par personne. Une valeur hors du domaine physique produit un **avertissement**, jamais un écrêtage silencieux — écrêter retirerait à l'ingénieur la décision qu'il vient de prendre. Le forçage **ne s'enregistre pas** : c'est une hypothèse d'examen, pas une donnée d'ouvrage. La méthode et la convention, elles, s'enregistrent.

  Le cas de validation de `SPEC_correction_Asmin_bridage.md` §7 — **27,44 cm²/m par nappe** — est vérifié dans `tests/detailing/restraint.test.ts`. C'est le **contrat commun** avec l'outil As,min de la suite : les deux implémentations sont séparées, aucun code n'est partagé, et c'est ce cas qui garantit qu'elles ne divergent pas.
